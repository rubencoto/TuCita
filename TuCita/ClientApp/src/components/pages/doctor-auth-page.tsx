...
import { Eye, EyeOff, Mail, Lock, Stethoscope, AlertCircle, ArrowLeft, Settings } from 'lucide-react';
  ...
  const handleLogin = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (selectedRole === 'admin') {
        ...
        await onLogin(adminData);
        // App will handle navigation after setting auth state
        return;
      }
      ...
      <div className="w-full max-w-md">
          ...
          <CardContent>
            ...
            <form onSubmit={handleLogin} className="space-y-5">
              ...
              <div className="flex items-center justify-between text-sm">
                ...
                <button type="button" className="text-[#2E8BC0] hover:underline" onClick={() => onNavigate('forgot-password')} disabled={loading}>
                  ¿Olvidaste tu contraseña?
                </button>
              </div>

              <Button type="submit" className="w-full bg-[#2E8BC0] hover:bg-[#2E8BC0]/90" disabled={loading}>
                {selectedRole === 'admin' ? <Settings className="h-4 w-4 mr-2" /> : <Stethoscope className="h-4 w-4 mr-2" />}
            ...
            <div className="mt-6 space-y-4">
              ...
              <div className="text-center space-y-3">
                <button type="button" onClick={() => onNavigate('login')} className="text-sm text-gray-600 hover:text-gray-900 flex items-center justify-center w-full" disabled={loading}>
                  <ArrowLeft className="h-4 w-4 mr-2" />
                  Volver al login de pacientes
                </button>

                <div className="pt-2 border-t">
                  <p className="text-sm text-gray-600 mb-2">¿Aún no estás registrado como médico?</p>
                  <button type="button" className="text-sm text-[#2E8BC0] hover:underline font-medium" onClick={() => onNavigate('register', { role: selectedRole })} disabled={loading}>
            ...
            <div className="mt-6 p-4 bg-gray-50 rounded-lg border border-gray-200">
              <p className="text-xs text-gray-500 text-center">
                <strong>Aviso de Seguridad:</strong> Este portal maneja información médica confidencial. Tu sesión está protegida con encriptación de extremo a extremo.
              </p>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="mt-4 bg-yellow-50 border-yellow-200">
          <CardContent className="pt-4">
            <p className="text-xs text-yellow-800 mb-2"><strong>?? Credenciales de prueba:</strong></p>
            <div className="space-y-2 text-xs text-yellow-700">
              <div>
                <strong>Doctor:</strong><br />Email: doctor@tucitaonline.com<br />Contraseña: demo123
              </div>
              <div className="pt-2 border-t">
                <strong>Admin:</strong><br />Email: admin@tucitaonline.com<br />Contraseña: admin123
              </div>
            </div>
          </CardContent>
        </Card>
